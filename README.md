...
```ts
type injectSpritesInTemplates = boolean;
```

Default: `false`

Enables the plugin to inject sprites into HTML files generated by [html-webpack-plugin](https://github.com/jantimon/html-webpack-plugin). The sprites will be injected at the beginning of the content `<body>`.

> [!NOTE]
> The sprites associated with the entrypoints listed in the `chunks` parameter of HtmlWebpackPlugin will be automatically included in the corresponding HTML files.
