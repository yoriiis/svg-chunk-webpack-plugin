...
class SvgChunkWebpackPlugin {
	...
	async handleHtmlWebpackPlugin(compilation: Compilation, sprites: Array<Sprite>): Promise<void> {
		...
		if (HtmlWebpackPlugin) {
			HtmlWebpackPlugin.getCompilationHooks(compilation).afterTemplateExecution.tapAsync(
				{ name: 'SvgChunkWebpackPlugin' },
				this.injectSpriteInHtmlWebpackPluginTemplate.bind(this, sprites)
			);
		}
	}
	 ...
	 * @param {Compilation} compilation Webpack compilation
	 * @returns HtmlWebpackPlugin instance or null
	 */
	getHtmlWebpackPluginInstance(compilation: Compilation): any | null {
		return (
